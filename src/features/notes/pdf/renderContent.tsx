// src/features/notes/pdf/renderContent.tsx
// Maps TipTap JSON nodes → @react-pdf/renderer primitives.
...
function resolveTextStyle(marks: any[]): Record<string, any> {
  ...
  for (const mark of marks ?? []) {
    switch (mark.type) {
      case "bold":
        style.fontFamily = style.fontStyle === "italic" ? "CrimsonPro-BoldItalic" : "CrimsonPro-Bold";
        break;
      case "italic":
        style.fontStyle  = "italic";
        style.fontFamily = style.fontFamily === "CrimsonPro-Bold" ? "CrimsonPro-BoldItalic" : "CrimsonPro-Italic";
        break;
      case "underline":
...
export function renderNodes(nodes: any[], noteStore?: any): any[] {
  return (nodes ?? []).map((node, i) => renderNode(node, i, noteStore));
}
  ...
  switch (node.type) {
    case "text": {
      ...
      if (linkMark?.attrs?.href) {
        return <Link key={key} src={linkMark.attrs.href} style={style}>{node.text}</Link>;
      }
      return <Text key={key} style={style}>{node.text}</Text>;
    }

    case "paragraph": {
      const children = (node.content ?? []).map((c: any, i: number) => renderNode(c, i, noteStore));
      if (children.length === 0) return <View key={key} style={{ height: 8 }} />;
      return <View key={key} style={contentStyles.paragraph}>{children}</View>;
    }
    ...
    case "blockquote":
      return (
        <View key={key} style={contentStyles.blockquote}>
          {(node.content ?? []).map((c: any, i: number) => renderNode(c, i, noteStore))}
        </View>
      );
    ...
    case "codeBlock":
      return (
        <View key={key} style={contentStyles.codeBlock}>
          <Text style={{ fontFamily: "JetBrainsMono", fontSize: 9.5, color: "#e8e0d5", lineHeight: 1.5 }}>
            {extractText(node)}
          </Text>
        </View>
      );
          ...
          {(node.content ?? []).map((item: any, i: number) => (
            <View key={i} style={contentStyles.bulletListItem}>
              ...
              <View style={contentStyles.listContent}>
                {(item.content ?? []).map((c: any, j: number) => renderNode(c, j, noteStore))}
              </View>
            </View>
          ))}
        </View>
    ...
    case "image":
      ...
      return <Image key={key} src={node.attrs.src} style={contentStyles.image} />;
    ...
    case "syncedBlock": {
      ...
      const sourceNote  = noteStore
        ? (noteStore.notes ?? []).find((n: any) => n.id === sourceNoteId)
        : null;
      const doc         = sourceNote?.content ?? sourceNote?.body;
      const sourceBlock = typeof doc === "object"
        ? (doc?.content ?? []).find((n: any) => n.attrs?.blockId === blockId)
        : null;
      return (
        <View key={key} style={contentStyles.syncedBlock}>
          <Text style={contentStyles.syncedLabel}>synced · {sourceNote?.title ?? "Unknown note"}</Text>
          {sourceBlock
            ? renderNode(sourceBlock, 0, noteStore)
            : <Text style={{ fontSize: 10, color: "#888", fontFamily: "CrimsonPro" }}>[Block not found]</Text>}
        </View>
      );
    }
    ...
    case "youtube":
      return (
        <View key={key} style={{ ...contentStyles.blockquote, borderLeftColor: "#e05c5c" }}>
